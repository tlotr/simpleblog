...
if($last != 1){
	if ($pagenum > 1){
		$previous = $pagenum - 1;
		$paginationCtrls .= '<a href="http://localhost/simpleblog/page/'.$previous.'"><li class="pagi_left">Previous</li></a>';
		for($i = $pagenum-4; $i < $pagenum; $i++){
			if($i > 0){
				$paginationCtrls .= '<a href="http://localhost/simpleblog/page/'.$i.'"><li>'.$i.'</li></a>';
			}
		}
	}
	...
	if ($pagenum != $last) {
		$next = $pagenum + 1;
		$paginationCtrls .= '<a href="http://localhost/simpleblog/page/'.$next.'"><li class="pagi_right">Next</li></a>';
	}
	$paginationCtrls .= '</ul>';
}
$posts = array();
...
<tr>
<td class='left'>
<p class="user"><a href="http://localhost/simpleblog/user/<?php echo $post['u_id']; ?>"><?php echo "<span class='underline'>". $post['fname'] ." ". $post['lname'] ."</span>"; ?></a></p><br />
